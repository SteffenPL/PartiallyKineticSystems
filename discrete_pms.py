...
class SymbolicsPMS:
    ...
    def __init__(self, T_r, U_r, T_q, U_q, g):
        ...
        self.F_mod_meso = lambda r, dr, rho, qx, qw: F_r(r,dr) + sum( Dpsi(r,qx) * F_int(r,dr,qx) * rho * qw  ) 
...
class DiscretePMS:
    ...
    def simulate_meso(self, method="BDF", atol=1.e-5, rtol=1.e-6, G=None, use_upwind=True):
        ...
        def laxfriedrichs(v, rho, dqgrid):
            
            drho = np.zeros_like(rho)
            
            drho[inner] -= v[inner]/(2*dqgrid[0:-1])*(rho[right] - rho[left]) - 1./2. * ( rho[left] - 2*rho[inner] + rho[right] )  # * (rho[inner] > 0.5)
        
            return drho
        
        disc_advection = laxfriedrichs
        
        if use_upwind:
            disc_advection = upwind
        ...
        def ode_fun_meso(t, y):
            ...
            return np.concatenate( [np.array([dr, F/M]), disc_advection(v, rho, dqgrid)] )
        ...
        if G is not None:
            ...
            def G_func(y, t):
                ...
                return np.concatenate( [np.array([0, G(y,t)/M]), disc_advection(v, rho, dqgrid)] )
    ...
    def simulate_meso_own(self, dt = 1.e-2, atol=1.e-5, rtol=1.e-6):
        ...
        n_eval = int(np.ceil(self.t_end/dt)) + 2
        
        self.r = np.zeros(shape=(1,n_eval))
        self.dr = np.zeros(shape=(1,n_eval))
        self.rho = np.zeros(shape=(len(rho),n_eval))
        ...
        self.sol = lambda : 0
        self.sol.t = np.arange(0,self.t_end+dt,dt)
        self.sol.y = np.concatenate( [self.r, self.dr, self.rho] , axis = 0 )
    ...
    def plot_heavy_system(self):
        ...
        plt.xlabel("t"), plt.ylabel("r"), plt.title("Macroscopic System")
    ...
    def plot_particle_paths(self, nParticles = 500, use_r_axis=False, plot_singular_pts=False, lw=0.3, alpha=1.):
        ...
        plt.ylabel("q"), plt.title("Cross-Bridge Extensions")
    ...
    def plot_particle_paths_meso_time(self, plot_singular_pts=False):
        ...
        plt.xlabel("t"), plt.ylabel("q"), plt.title("Cross-Bridge Distribution")

            
    def plot_energies(self, energies, energy_colors, str_sim_type=""):
    
        def plot_energy(k,linestyle="-"):
            plt.plot(self.sol.t, energies[k], color=energy_colors[k], linestyle=linestyle)
            
        def plot_energy_sum(k,l):
            plt.plot(self.sol.t, energies[k] + energies[l], color=energy_colors[4])
        ...
        plt.title("Attached Cross-Bridges")
        ...
        for k in range(5):
            plot_energy(k)
        plt.xlabel("t"), plt.ylabel("Energy")
        plt.title("Coupled System " + str_sim_type)
    ...
    def calc_energies(self, show_plot=False):
        ...
        if show_plot:
            self.plot_energies(energies, energy_colors, "(DAE Model)")
            
            plt.legend([r'$E^{\mathrm{kin}}_r$',r'$E^{\mathrm{pot}}_r$',
                        r'$E^{\mathrm{kin}}_{Q_1,\dots,Q_N}$',r'$E^{\mathrm{pot}}_{Q_1,\dots,Q_N}$',r"$E^{\mathrm{total}}$"],
                        loc='lower left', bbox_to_anchor=(0.05,-0.19),
    ...
    def calc_energies_meso(self, show_plot=False):
        ...
        if show_plot:
            self.plot_energies(energies, energy_colors, "(Partially Kinetic Model)")
                
            plt.legend([r'$E^{\mathrm{kin}}_r$',r'$E^{\mathrm{pot}}_r$',
                        r'$E^{\mathrm{kin}}_{\mu^t}$',r'$E^{\mathrm{pot}}_{\mu^t}$',r"$E^{\mathrm{total}}$"],
                        loc='lower left', bbox_to_anchor=(0.08,-0.19),
                        bbox_transform=plt.gcf().transFigure, ncol=3)
