...
class DiscretePMS:
    ...
    def simulate_meso_along(self, disc_t, disc_r, disc_dr, dt = 1.e-2, atol=1.e-5, rtol=1.e-6,  method="BDF"):
        ...
        def get_r(t):
            i = np.searchsorted(disc_t,t)
            if i > 0:
                if i < len(disc_t):
                    
                    t_found = disc_t[i]
                    diff_t = disc_t[i] - t_found 
                    step = disc_t[i] - disc_t[i-1]
                    p = diff_t / step
                
                    return (p*disc_r[i-1] + (1.-p)*disc_r[i], p * disc_dr[i-1] + (1.-p) * disc_dr[i])
                else:            
                    return (disc_r[i-1], disc_dr[i-1])
        
            return (disc_r[i], disc_dr[i])
        ...
        def ode_fun_meso(t, y):
            ...
            return disc_advection(v, rho, dqgrid)
    
        self.sol = solve_ivp( ode_fun_meso, [0,self.t_end], self.y0_meso[2:] , method=method, atol=atol, rtol=rtol, t_eval=self.t_eval)
